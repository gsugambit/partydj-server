...
drop table if exists station_queue_item;
alter table station drop constraint UK_gnneuc0peq2qi08yftdjhy7ok unique (name)
alter table station_queue_item drop constraint UK_5jo1jx00lk32ivlht00k2dbkc unique (queue_item_id)
alter table queue_item drop constraint FK4pajiom1uov9u98qkv3w7nbs9 foreign key (station_station_id) references station (station_id)
alter table station_queue_item drop constraint FK7t390r5i95brtdrjiqn3l8yv3 foreign key (queue_item_id) references queue_item (queue_item_id)
alter table station_queue_item drop constraint FK3n2mv2jhfxdfms93ie82vgxta foreign key (station_id) references station (station_id)
